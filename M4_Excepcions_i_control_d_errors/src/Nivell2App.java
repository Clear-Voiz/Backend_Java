...
public class Nivell2App {
	...
	public static void main(String[] args) {
		...
		//variables pels bitllets
				...
				while(seguir == 1) { //inici búcle de la comanda
					...
					System.out.println("Què voldrà prendre? (introdueixi el nom del plat)");
						...
						if (existeix == false) { //si el plat no existeix llança una excepció
							throw new ArrayIndexOutOfBoundsException("El plat indicat no es troba en el nostre menú");
						}
						} catch(Exception e) {
							System.out.println("El plat indicat no es troba en el nostre menú");
						}
					...
					} catch (InputMismatchException e) { //en cas que s'introdueixi un valor que no sigui byte s'encetarà un bucle while que seguirà demanant que es corregeixi amb un valor vàlid
						intro = 7;
						while(intro == 7) {
				...
				if(total>0) { //en cas que encarà sobrés total per pagar i fos més petit que 5 (el bitlle més petit), s'afegiria un bitllet més de 5 al ser el més petit possible i se'ns hauria de tornar canvi
					b5 += 1;
					total = 0;
				}
